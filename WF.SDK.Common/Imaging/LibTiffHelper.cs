...
namespace WF.ImageTools
{
	public class LibTiffHelper
	{
		public static int GetTiffPageCount(string[] filePaths)
		{
			int count = 0;
			foreach (string filePath in filePaths)
			{
				using (Tiff tiffImage = Tiff.Open(filePath, "r"))
				{
					count += tiffImage.NumberOfDirectories();
				}
			}
			return count;
		}
			...
			try
			{
				...
				using (MemoryStream ms = new MemoryStream())
				{
					using (Tiff tif = Tiff.ClientOpen("InMemory", "w", ms, new TiffStream()))
					{
						...
						tif.SetField(TiffTag.COMPRESSION, Compression.CCITTFAX3);
